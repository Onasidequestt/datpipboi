#!/usr/bin/env python3
import sys, pathlib; sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent / "src"))  # noqa: E402 — flat-import shim
"""
...
reg = L.registry()
if reg.get("runner") and reg.get("vacc"):
    ...
    # Lane spec JSONs (src/lanes/*.json) are operator-local strategy config, not shipped
    # in this template. The verdict-math coverage below is self-contained and still runs.
    print("  ⏭ lanes/ registry not shipped in this template — skipping 8 registry checks")
